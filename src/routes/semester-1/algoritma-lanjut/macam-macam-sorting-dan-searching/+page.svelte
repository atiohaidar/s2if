<script lang="ts">
  ...
  const materiQuiz = [
    {
      question:
        'Pada selection sort, jumlah perbandingan utama untuk n elemen paling tepat adalah...',
      options: ['n', 'n log n', '(n-1) + (n-2) + ... + 1', '2^n'],
      correctIndex: 2,
      explanation:
        'Di tiap putaran i, kita scan sisa elemen untuk cari minimum, sehingga totalnya deret 1 sampai n-1.',
    },
    {
      question:
        'Algoritma yang biasanya paling cocok untuk data kecil dan hampir terurut adalah...',
    ...
    },
    {
      ...
      correctIndex: 1,
      explanation:
        'Binary search butuh prasyarat data terurut. Kalau belum, harus sorting dulu atau pakai linear search.',
    },
    {
...
<article class="note-article">
  ...
  <NoteSection title="Kenapa Topik Ini Penting?">
    ...
    <Callout type="info" title="Mindset cepat">
      Saat ketemu soal, tanyakan dulu: "Data perlu diurutkan dulu atau tidak?" Dari pertanyaan ini,
      kamu biasanya bisa memilih strategi utama dengan cepat.
    </Callout>
  </NoteSection>

  <NoteSection title="A. Sorting Dibahas Bertahap (Bukan Digabung)">
    <p>
      Mulai dari algoritma pertama dulu, paham cara kerja dan analisisnya, baru pindah ke algoritma
      berikutnya.
    </p>
    <Callout type="info" title="Alur belajar di halaman ini">
      Untuk setiap algoritma: konsep -> kelebihan/kekurangan -> kode Python -> contoh hasil ->
      kompleksitas + cara menurunkannya.
    </Callout>
  </NoteSection>

  <NoteSection title="1) Selection Sort">
    <p>
      <strong>Konsep:</strong> pada setiap posisi <code>i</code>, cari elemen minimum dari bagian
      kanan, lalu tukar ke posisi <code>i</code>.
    </p>
...
dst.`}
  ...
  <NoteSection title="2) Bubble Sort">
    <p>
      <strong>Konsep:</strong> bandingkan elemen bersebelahan, tukar jika salah urut. Setelah satu pass,
      elemen terbesar "naik" ke ujung kanan.
    </p>
...
Output: [1, 2, 4, 5]`}
  ...
  <NoteSection title="3) Insertion Sort">
    <p>
      <strong>Konsep:</strong> anggap bagian kiri sudah terurut, lalu sisipkan elemen berikutnya ke posisi
      yang tepat.
    </p>
...
Output: [5, 6, 11, 12, 13]`}
  ...
  <NoteSection title="4) Merge Sort">
    <p>
      <strong>Konsep:</strong> bagi array jadi dua bagian sampai elemen tunggal, lalu gabungkan kembali
      secara terurut (divide and conquer).
    </p>
...
Algoritma function merge(left, right) -> array:
    ...
    <Callout type="info" title="Kenapa ada dua while tambahan di akhir?">
      Looping utama <code>while i &lt; len(left) and j &lt; len(right)</code> akan langsung berhenti
      ketika salah satu blok (Kiri atau Kanan) habis dimasukkan ke <code>result</code>. Ini
      menyebabkan blok lawannya mungkin masih memiliki sisa elemen yang <strong>tertinggal</strong>.
      Dua looping tambahan di bagian akhir bertugas menyapu bersih sisa elemen tersebut agar tidak
      ada data yang hilang!
    </Callout>

    <Callout type="tip" title="Bagaimana array Kiri dan Kanan dipastikan selalu terurut?">
      Fungsi <code>merge</code> mensyaratkan kedua array input sudah terurut. Lalu bagaimana cara
      memastikannya? Algoritma ini membelah array terus-menerus sampai ukurannya tersisa
      <strong>1 elemen</strong>
      (<code>if len(arr) &lt;= 1</code>). Secara logika, array berisi 1 elemen sudah pasti terurut
      sempurna! Dari kepingan-kepingan terkecil inilah proses penggabungan bergerak mundur perlahan
      ke atas menjadi ukuran 2, 4, 8, dan seterusnya.
    </Callout>

    <Callout type="info" title="Apa untungnya menggabungkan array yang sudah terurut?">
      Keuntungannya luar biasa besar pada efisiensi jumlah perbandingan.<br /><br />
      Bayangkan dua barisan angka <strong>acak</strong>: <code>[18, 15, 17]</code> dan
      <code>[16, 19, 11]</code>. Untuk mencari nilai terkecil dan menyusun ulang keduanya, kita
      harus mengecek semua angka berulang kali karena nilai terkecil bisa bersembunyi di posisi
      manapun. Ini sangat lambat dan memakan waktu <code>O(N²)</code>.<br /><br />
      Tapi, bayangkan jika kelompoknya <strong>sudah terurut</strong>: <code>[15, 17, 18]</code> dan
      <code>[11, 16, 19]</code>. Karena sudah terurut, kita <strong>pasti tahu</strong> bahwa elemen
      terkecil masing-masing kelompok ada di posisi paling depan! Kita cukup membandingkan yang
      terdepan saja (<code>15 vs 11</code>). Ambil yang terkecil (11), lalu lanjut membandingkan
      pemenang selanjutnya (<code>15 vs 16</code>). Karena kita hanya melirik setiap elemen bergeser
      maju tepat <strong>satu kali</strong>, proses penggabungan ini menjadi sangat cepat
      <code>O(N)</code>.
    </Callout>
...
Output: [3, 9, 10, 27, 38, 43, 82]`}
  ...
  <NoteSection title="5A) Quick Sort (Lomuto Partition)">
    <p>
      <strong>Konsep:</strong> pilih pivot (biasanya ujung kanan), partisi elemen kecil ke kiri, dengan
      pointer pencari (j) bergerak searah dari kiri ke kanan.
    </p>
    <p><strong>Kelebihan:</strong> sangat cepat di praktik rata-rata, in-place.</p>
    <p>
      <strong>Kekurangan:</strong> jika pivot buruk berulang, bisa turun ke <code>O(n^2)</code>.
    </p>
...
Output: [1, 5, 7, 8, 9, 10]`}
  ...
  <NoteSection title="5B) Quick Sort (Hoare Partition)">
    <p>
      <strong>Konsep:</strong> Menggunakan teknik partisi dua arah. Memiliki pointer di kiri dan kanan
      yang bergerak ke tengah. Lebih efisien dalam jumlah pertukaran (swap) dibanding Lomuto, sehingga
      sering digunakan di implementasi standar dan diajarkan di kelas kampus lanjutan.
    </p>
...
Algoritma function partition_hoare(a, low, high) -> integer:
    ...
    <Callout type="info" title="Analogi Hoare Partition: Dua Satpam Saling Mendekat">
      ...
      <strong>"angka yang salah baris karena terlalu besar"</strong>. Satpam Kanan (<code>r</code>)
      jalan pelan ke kiri menahan <strong>"angka yang salah baris karena terlalu kecil"</strong>.<br
      /><br />
      Ketika Satpam Kiri menemukan angka besar, dia berhenti. Dia menunggu sampai Satpam Kanan juga menemukan
      angka kecil. Saat dua-duanya sudah menemukan "pelanggar", mereka saling melempar/menukar silang
      angka tersebut! Proses ini diulang terus sampai kedua satpam bertemu/bersilangan di tengah lapangan.
      Di titik pertemuan itulah sang Pivot (Patokan) akhirnya ditempatkan!
    </Callout>
  </NoteSection>

  <NoteSection title="6) Heap Sort">
    <p>
      <strong>Konsep:</strong> ubah array menjadi <strong>max-heap</strong>, lalu tukar root
      (terbesar) ke belakang array secara bertahap.
    </p>
    <p><strong>Kelebihan:</strong> worst case tetap <code>O(n log n)</code> dan in-place.</p>
    <p>
      <strong>Kekurangan:</strong> tidak stabil, dan biasanya lebih sulit dipahami dari merge sort.
    </p>
...
Output: [5, 6, 7, 11, 12, 13]`}
  ...
  <NoteSection title="7) Counting Sort">
    <p>
      <strong>Konsep:</strong> hitung frekuensi setiap nilai, lalu bangun ulang array berdasarkan jumlah
...
Output: [1, 2, 2, 3, 3, 4, 8]`}
  ...
  <NoteSection title="B. Searching Dibahas Bertahap">
    <p>Sama seperti sorting, searching juga dipahami satu per satu biar kebayang kapan dipakai.</p>
  </NoteSection>

  <NoteSection title="1) Linear Search">
    <p>
      <strong>Konsep:</strong> cek elemen dari kiri ke kanan sampai target ketemu atau data habis.
    </p>
...
Output: 2`}
  ...
  <NoteSection title="2) Binary Search">
    <p>
      <strong>Konsep:</strong> bandingkan target dengan elemen tengah, lalu buang setengah ruang pencarian
      tiap langkah.
    </p>
...
ketemu 16`}
  ...
  <NoteSection title="Cara Memahami Materi Ini Biar Nempel">
    <ol>
      ...
      <li>
        Untuk tiap algoritma, tulis 5 hal: konsep, plus-minus, kode, contoh hasil, kompleksitas.
      </li>
  ...
  <BackLink href="/semester-1/algoritma-lanjut" label="Kembali ke Desain Algoritma Lanjut" />
</article>
